    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if checkout_session:
            ...
            if user:
                ...
                # Updated container creation with -it functionality
                container = docker_client.containers.run(
                    ...
                    environment={
                        "EULA": "TRUE",
                        "ENABLE_RCON": "true",  # If you want to enable RCON
                        "RCON_PASSWORD": "your_rcon_password",  # Set RCON password
                        "RCON_PORT": str(port + 1000)  # Example RCON port
                    },
                    ports={'25565/tcp': port},
                    name=container_name,
                    mem_limit=memory_limit
                )
                add_container_to_db(container_name, port, 'running', price_id_to_plan_name.get(price_id, 'Unknown Plan'), container.short_id, user.id)
                return 'Success', 200
        else:
...
def configure_server(server_id):
    # Add your logic here for server configuration
    return render_template('configure_server.html', server_id=server_id)

@app.route('/terminal_server/<int:server_id>')
@login_required
def terminal_server(server_id):
    # Add your logic here for terminal access
    return render_template('terminal_server.html', server_id=server_id)
